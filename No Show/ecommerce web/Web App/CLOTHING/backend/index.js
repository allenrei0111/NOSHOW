...
// Example promo codes with their discount percentages
const promoCodes = {
    "SAVE10": 0.10, // 10% discount
    "GET20OFF": 0.20, // 20% discount
    "FREESHIPPING": 1 // Free shipping (100% discount)
};

// Endpoint to apply promo code
app.post('/applypromo', (req, res) => {
    try {
        const { promoCode } = req.body;
        let discountPercentage = 0;
        let message = "";

        // Check if the promo code exists in the promoCodes object
...
// Endpoint to remove promo code
app.post('/removepromo', (req, res) => {
    try {
        // Placeholder logic to remove promo code from the cart
        // You need to implement your own logic here
        // Update the cart in the database accordingly
        res.status(200).json({ success: true, message: "Promo code removed successfully" });
    } catch (error) {
        res.status(500).json({ success: false, message: "Failed to remove promo code" });
    }
});
...
app.get("/popularinallcategories", async (req, res) => {
    let products = await Product.find({});
    let arr = products.splice(0, 4);
    console.log("Popular In All Categories");
    res.send(arr);
});
